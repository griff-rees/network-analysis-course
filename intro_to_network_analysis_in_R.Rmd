...
To see how that shows up when loaded into a `igraph` network use the `V` (vertices, synonymous with nodes) function:

```{r, eval=False}
V(g)
```

The `name` attribute is particularly helpful in graph visualisation as it shows up automatically with `igraph` plots. If you've not used `R` before, here's a useful way to look at parts of data (usually columns in a table) using the `$`.
...
```{r}
# Add the color attribute to the network nodes
V(h)$color <- NA  # Initialse the new 'color' attribute as NA for all nodes
```
...
```{r}
V(h)$color[V(h)$name %in% dark_side] <- "red" # set the dark side color name to red
```

To break this down: the `%in%` opeation tests if data in one column---in this case the `$name` variable which is to the *left* of `%in%`---matches with (*is in*) the options on the *right* of the `%in%`.

```{r}
V(h)$color[V(h)$name %in% light_side] <- "gold" # set the light side color name to gold
V(h)$color[V(h)$name %in% neutral] <- "green" # set the color of neutral characters to green
```
Where it does match it returns `TRUE` and in this case that means the colour value intended gets saved in places where they match the dark, light and neural sides. It's complicated to explain but extremely useful.
